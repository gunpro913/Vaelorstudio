...
export default function Anatomy() {
  ...
  return (
    <section ref={containerRef} className="py-24 md:py-48 px-6 md:px-12 bg-aer-charcoal relative border-t border-aer-cream/5 min-h-[80vh] flex flex-col justify-center overflow-hidden">
      <div className="max-w-7xl w-full mx-auto flex flex-col relative">
        ...
        <motion.div style={{ y }} className="w-full flex flex-col items-center justify-center relative mt-24 md:mt-24">
          
          {/* Flex container fixed for mobile and glitch-free hovering */}
          <div 
            className="flex flex-col md:flex-row items-center justify-center font-editorial text-[22vw] md:text-[11vw] leading-none uppercase z-10 cursor-crosshair w-full"
          ...
          <div className="h-48 md:h-48 w-full max-w-2xl mt-12 md:mt-16 relative flex items-start justify-center">
            <AnimatePresence mode="wait">
              {hovered ? (
                <motion.div
                  ...
                  className="flex flex-col items-center text-center gap-6 absolute w-full px-4"
                >
                  <div className="flex items-center gap-4 border-b border-aer-cream/20 pb-4">
                    <span className="font-editorial text-2xl md:text-3xl text-aer-cream tracking-[0.2em]">{definitions[hovered as keyof typeof definitions].title}</span>
                    <span className="text-[10px] tracking-[0.2em] text-aer-blue font-mono uppercase">{definitions[hovered as keyof typeof definitions].phonetic}</span>
                  </div>
                ...
                >
                  <p className="text-[10px] tracking-[0.3em] text-aer-cream/30 uppercase animate-pulse">
                    [ HOVER TO DECODE ]
                  </p>
                </motion.div>
              )}
