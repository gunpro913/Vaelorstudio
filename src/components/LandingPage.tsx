...
import { ArrowDownRight, ArrowUpRight, CircleArrowUpRight, Plus, Sparkles } from 'lucide-react';

const projects = [
  { index: '01', type: 'COMMERCE / SYSTEMS', title: 'Velora', text: 'A commerce experience engineered around clarity, confidence and conversion.', size: 'large' },
  { index: '02', type: 'BRAND / EXPERIENCE', title: 'Nimble', text: 'A sharper digital identity for a new generation of creators.', size: 'small' },
  { index: '03', type: 'PRODUCT / INTERFACE', title: 'Flux', text: 'A product system for real-time insight and decisive action.', size: 'small' },
];
    ...
    <div className="relative h-full min-h-[280px] overflow-hidden rounded-[22px] border border-white/10 bg-[#0d1112]">
      ...
      <div className="absolute inset-8 rounded-[14px] border border-white/10 bg-black/35 shadow-[0_30px_80px_rgba(0,0,0,.55)] backdrop-blur-sm">
        {variant === 0 ? (
          ...
          <div className="absolute inset-0 p-5"><div className="flex justify-between text-[7px] uppercase tracking-[.25em] text-white/40"><span>{variant === 1 ? 'NIMBLE / BRAND' : 'FLUX / PRODUCT'}</span><span>LIVE</span></div><div className="absolute left-8 right-8 top-1/2 -translate-y-1/2"><div className="font-editorial text-4xl text-white/90">{variant === 1 ? 'Make room.' : 'See clearly.'}</div><div className="mt-5 grid grid-cols-3 gap-2"><span className="h-14 rounded border border-white/10 bg-white/[.03]" /><span className="h-14 rounded border border-aer-blue/30 bg-aer-blue/[.04]" /><span className="h-14 rounded border border-white/10 bg-white/[.03]" /></div></div></div>
        )}
      </div>
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#0a0b0b] text-aer-cream selection:bg-aer-blue selection:text-black">
      ...
      <header className="fixed left-0 right-0 top-0 z-40 border-b border-white/[.06] bg-[#0a0b0b]/75 backdrop-blur-xl">
        <div className="mx-auto flex h-[74px] max-w-[1500px] items-center justify-between px-5 md:px-10">
          <a href="#top" className="text-[11px] font-medium tracking-[.3em] text-white">AER <span className="text-aer-blue">×</span> VÆLOR</a>
          <nav className="hidden items-center gap-8 md:flex">
            {['WORK', 'CAPABILITIES', 'PROCESS', 'STUDIO', 'LAB'].map((item) => <a key={item} href={`#${item.toLowerCase()}`} className="text-[8px] tracking-[.25em] text-white/55 transition-colors hover:text-white">{item}</a>)}
          </nav>
          <a href="#contact" className="group flex items-center gap-3 rounded-full border border-white/20 px-4 py-2 text-[8px] uppercase tracking-[.22em] transition hover:border-aer-blue/70 hover:bg-aer-blue/10">Start a project <Signal /></a>
        </div>
      </header>

      <main id="top" className="relative z-10">
        <section className="relative flex min-h-screen items-end overflow-hidden border-b border-white/[.07] px-5 pb-16 pt-32 md:px-10 md:pb-24">
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_70%_45%,rgba(120,210,225,.16),transparent_22%),radial-gradient(ellipse_at_82%_30%,rgba(255,255,255,.13),transparent_13%),linear-gradient(110deg,#0a0b0b_30%,#081215_62%,#0a0b0b)]" />
          <div className="absolute right-[13%] top-[13%] h-[48vw] w-[20vw] max-h-[640px] rounded-[50%_50%_6%_6%] border border-white/20 bg-black/20 shadow-[0_0_100px_rgba(64,224,208,.09)]" />
          <div className="absolute right-[14%] top-[16%] h-[42vw] w-[17vw] max-h-[560px] rounded-[50%_50%_3%_3%] bg-[radial-gradient(ellipse_at_70%_25%,rgba(255,255,255,.5),transparent_18%),linear-gradient(90deg,transparent,rgba(64,224,208,.08),transparent)] blur-[1px]" />
          <div className="absolute left-[7%] right-[7%] top-[24%] h-px bg-white/[.06]" /><div className="absolute bottom-[19%] left-[7%] right-[7%] h-px bg-white/[.06]" />
          <div className="relative mx-auto w-full max-w-[1500px]">
            <div className="grid items-end gap-12 md:grid-cols-[1.1fr_.55fr]">
              <div>
                <SectionLabel>Digital studio / 001</SectionLabel>
                <motion.h1 {...reveal()} className="max-w-5xl font-editorial text-[clamp(4.2rem,9.2vw,10rem)] leading-[.82] tracking-[-.045em]">Ideas, engineered<br /><span className="text-white/35">for what’s next.</span></motion.h1>
                <motion.p {...reveal(.12)} className="mt-9 max-w-xl text-sm leading-7 text-white/55 md:text-base">AER × VÆLOR is a digital studio building intelligent experiences for ambitious brands. Strategy, design and technology — shaped into products that move.</motion.p>
                <motion.a {...reveal(.2)} href="#work" className="mt-8 inline-flex items-center gap-4 rounded-full border border-white/20 px-5 py-3 text-[9px] uppercase tracking-[.28em] transition hover:border-aer-blue hover:bg-aer-blue hover:text-black">Explore our work <ArrowDownRight size={14} /></motion.a>
              </div>
              <div className="hidden pb-3 md:block"><div className="ml-auto max-w-[180px] border-l border-white/10 pl-5 text-[8px] uppercase leading-5 tracking-[.25em] text-white/35">Strategy<br />Design<br />Motion<br />Engineering<br /><span className="text-aer-blue">—</span><br />Systems that feel inevitable.</div></div>
            </div>
            <div className="mt-16 flex items-center justify-between text-[8px] uppercase tracking-[.25em] text-white/30"><span>Scroll to explore</span><span>01 / 07 <span className="ml-3 text-aer-blue">●</span></span></div>
          </div>
        </section>

        <section id="work" className="mx-auto max-w-[1500px] px-5 py-28 md:px-10 md:py-36">
          <motion.div {...reveal()} className="mb-12 flex items-end justify-between gap-8"><div><SectionLabel>Selected work / 002</SectionLabel><h2 className="max-w-3xl font-editorial text-5xl leading-[.9] tracking-[-.03em] md:text-7xl">Digital experiences<br /><span className="text-white/35">that make an impact.</span></h2></div><span className="hidden text-[8px] uppercase tracking-[.25em] text-white/35 md:block">View all projects →</span></motion.div>
          <div className="grid gap-3 md:grid-cols-[1.35fr_.65fr]">
            <motion.article {...reveal()} className="group min-h-[560px] overflow-hidden rounded-[26px] border border-white/10 bg-[#0e1111] p-3 transition duration-500 hover:-translate-y-1 hover:border-aer-blue/40">
              <div className="relative h-full min-h-[540px]"><ProjectVisual variant={0} /><div className="absolute inset-x-7 bottom-7 flex items-end justify-between gap-6"><div><div className="mb-3 text-[8px] uppercase tracking-[.25em] text-aer-blue">01 / Commerce</div><h3 className="font-editorial text-4xl md:text-5xl">Velora</h3><p className="mt-2 max-w-md text-xs leading-5 text-white/45">A next-generation commerce platform built for speed, clarity and conversion.</p></div><CircleArrowUpRight className="shrink-0 text-white/50 transition group-hover:text-aer-blue" /></div></div>
            </motion.article>
            <div className="grid gap-3">
              {projects.slice(1).map((project, i) => <motion.article {...reveal(i * .08)} key={project.title} className="group relative min-h-[278px] overflow-hidden rounded-[26px] border border-white/10 bg-[#0e1111] p-3 transition duration-500 hover:-translate-y-1 hover:border-aer-blue/40"><ProjectVisual variant={i + 1} /><div className="absolute inset-x-8 bottom-7 flex items-end justify-between"><div><div className="mb-2 text-[8px] uppercase tracking-[.25em] text-aer-blue">{project.index} / {project.type}</div><h3 className="font-editorial text-3xl">{project.title}</h3></div><ArrowUpRight size={18} className="text-white/45 group-hover:text-aer-blue" /></div></motion.article>)}
            </div>
          </div>
        </section>

        <section id="capabilities" className="border-y border-white/[.07] bg-[#0d1010] px-5 py-28 md:px-10 md:py-36">
          <div className="mx-auto max-w-[1500px]"><motion.div {...reveal()} className="mb-12 flex items-end justify-between"><div><SectionLabel>Capabilities / 003</SectionLabel><h2 className="font-editorial text-6xl tracking-[-.04em] md:text-8xl">What we make.</h2></div><span className="hidden text-[8px] uppercase tracking-[.25em] text-white/30 md:block">The AER system</span></motion.div>
            <div className="grid border border-white/10 md:grid-cols-4">
              {capabilities.map(([num, title, text], i) => <motion.a {...reveal(i * .06)} href="#contact" key={num} className="group min-h-[300px] border-b border-white/10 p-7 transition hover:bg-white/[.025] md:border-b-0 md:border-r md:last:border-r-0"><div className="flex justify-between text-[8px] tracking-[.25em] text-white/35"><span>{num}</span><Signal className="opacity-30 transition group-hover:opacity-100" /></div><h3 className="mt-20 text-sm tracking-[.22em]">{title}</h3><p className="mt-4 max-w-[220px] text-xs leading-5 text-white/40">{text}</p><div className="mt-14 text-[8px] uppercase tracking-[.25em] text-aer-blue opacity-0 transition group-hover:opacity-100">Explore ↗</div></motion.a>)}
            </div>
          </div>
        </section>

        <section id="process" className="mx-auto max-w-[1500px] px-5 py-28 md:px-10 md:py-36">
          <div className="grid gap-16 lg:grid-cols-[.55fr_1fr]"><motion.div {...reveal()} className="lg:sticky lg:top-32 lg:self-start"><SectionLabel>Our process / 004</SectionLabel><h2 className="max-w-md font-editorial text-6xl leading-[.9] tracking-[-.04em] md:text-8xl">From insight<br /><span className="text-white/35">to impact.</span></h2><p className="mt-8 max-w-sm text-sm leading-6 text-white/40">A structured process built for better outcomes. Every stage earns the next.</p></motion.div><div className="space-y-3">{process.map(([num, title, text], i) => <motion.article {...reveal(i * .05)} key={num} className="group relative overflow-hidden rounded-[24px] border border-white/10 bg-[#0d1010] p-7 md:min-h-[220px] md:p-9"><div className="absolute right-8 top-8 text-[8px] tracking-[.25em] text-white/30">{num}</div><div className="max-w-lg"><div className="text-[9px] uppercase tracking-[.3em] text-aer-blue">{title}</div><h3 className="mt-7 font-editorial text-4xl md:text-5xl">{title === 'DISCOVER' ? 'Uncover the signal.' : title === 'DEFINE' ? 'Give it structure.' : title === 'DESIGN' ? 'Make the system visible.' : title === 'BUILD' ? 'Turn intent into product.' : 'Make every detail count.'}</h3><p className="mt-4 text-xs leading-6 text-white/40">{text}</p></div><div className="absolute bottom-0 left-0 h-px w-0 bg-aer-blue transition-all duration-500 group-hover:w-full" /></motion.article>)}</div></div>
        </section>

        <section id="studio" className="border-y border-white/[.07] bg-[#0d1010] px-5 py-28 md:px-10 md:py-36">
          <div className="mx-auto max-w-[1500px]"><SectionLabel>Our studio / 005</SectionLabel><div className="grid gap-10 lg:grid-cols-[1.1fr_.9fr]"><motion.h2 {...reveal()} className="font-editorial text-7xl leading-[.82] tracking-[-.045em] md:text-[9rem]">Design.<br />Build.<br /><span className="text-white/30">Evolve.</span></motion.h2><motion.div {...reveal(.1)} className="grid content-end gap-10"><p className="max-w-xl text-lg leading-8 text-white/55">We’re a small, focused team of designers and engineers. We believe in clarity, craftsmanship and systems that last.</p><div className="grid grid-cols-2 gap-x-8 gap-y-5 border-t border-white/10 pt-7 text-[9px] uppercase tracking-[.25em] text-white/35"><span>Typography / Editorial</span><span>Motion / Restrained</span><span>Color / Signal</span><span>Structure / Systemic</span><span>Detail / Intentional</span><span>Code / Precise</span></div></motion.div></div></div>
        </section>
